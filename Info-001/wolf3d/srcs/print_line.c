...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2014/05/16 13:02:17 by mrebours            #+#    #+#             */
/*   Updated: 2014/05/16 13:02:18 by mrebours           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_pixel_put_to_image(int x, int y,
			unsigned int color, t_infos_mlx *infos_mlx)
{
	void	*mlx_ptr;
	size_t	pixel_ptr;
...
void		print_line(t_line c, t_infos_mlx *infos_mlx, unsigned int color)
{
	...
	if ((c.x1 <= c.x2) && ((c.x2 - c.x1) >= val_abs(c.y2 - c.y1)))
	{
		...
		while (tmp <= c.x2)
		{
			ft_pixel_put_to_image(tmp, c.y1 + ((c.y2 - c.y1) * (tmp - c.x1))
	...
	while (tmp < c.y2)
	{
		ft_pixel_put_to_image(c.x1 + ((c.x2 - c.x1) * (tmp - c.y1))
